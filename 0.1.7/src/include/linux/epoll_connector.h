...
namespace netsvc{
	...
	int epoll_connector<handler>::push(handler *hd)
	{
        ...
        EV_SET(&kevent_ev, hd->sock(), EVFILT_WRITE, EV_ADD, 0, 0,(void*)hd);

        struct timespec time_out = {1, 0};
        if (kevent(_epoll_fd, &kevent_ev, 1, NULL, 0, 0, &time_out) == -1)
        {
            return -1;
        }
#else
	...
	void* epoll_connector<handler>::connect_thread(void *arg)
	{
        ...
        struct kqueue_event *events = new struct kevent[max_events];
        struct timespec time_out = {max_waittm/1000, 0};
        
        while (!connector->_connector_stop)
        {
            int num = kevent(connector->_epoll_fd , NULL, 0, events, max_events, &time_out);
            for (int i = 0; i < num; ++i)
            {
                handler *hd = (handler*)events[i].udata;
                
				if(events[i].flag & EV_ERROR)
				{
