...
namespace netsvc{
	...
	public:
		...
		/*start accepter with listen @port and @worker number, pass @arg to handler's recall method, @accept_timeout(ms) will used when accept.*/
		/*note: if @accept_timeout is too short, it will slow down accepter's efficiency; if @accept_timeout is very long, you will perhaps hard to stop the accepter while no client connect the accepter. if @accept_timeout is 0, accept will be non-block. if @accept_timeout is negative, accept will block until a client comes. so, I commend that, if the concurrency is supposed to be very big, @accept_timeout can be set with -1; otherwise, it can be set with a small positive number.*/
		int start(u_short port, int worker_num, void *arg = NULL, int accept_timeout = -1);

		/*start accepter with local @ip & listen @port and @worker number, pass @arg to handler's recall method, @accept_timeout(ms) will used when accept.*/
		int start(u_int ip, u_short port, int worker_num, void *arg = NULL, int accept_timeout = -1);
	...
	private:
		...
		/*thread function for accept socket from remote, can get timeout in accept*/
		static unsigned __stdcall timed_accept_thread(void *arg);
	...
	template<class handler>
	int iocp_accepter<handler>::start(u_short port, int worker_num, void *arg/* = NULL*/, int accept_timeout /* = -1 */)
	{
		return start(INADDR_ANY, port, worker_num, arg, accept_timeout);
	}

	template<class handler>
	int iocp_accepter<handler>::start(u_int ip, u_short port, int worker_num, void *arg/* = NULL*/, int accept_timeout /* = -1 */)
	{
		/*initial member*/
		_local_ip = ip;
		_listen_port = port;
		...
		{
			/*set accept timeout*/
			_timeout.tv_sec = accept_timeout / 1000;
			_timeout.tv_usec = (accept_timeout % 1000) * 1000;

			/*set listen socket to nonblock mode*/
			unsigned long io_mode = 1;
			int err = ioctlsocket(_listen_sock, FIONBIO, &io_mode);
			if(err != 0)
			{
			...
			/*use timed_accept_thread*/
			_hdl = (HANDLE)_beginthreadex(NULL, 0, timed_accept_thread, this, 0, &_thread_id);
		}
		else
		{
			/*use blocked_accept thread*/
	...
	template<class handler>
	unsigned iocp_accepter<handler>::timed_accept_thread(void *arg)
	{
		iocp_accepter<handler> *accepter = (iocp_accepter<handler> *)arg;
		...
		while (!accepter->_accepter_stop)
		{
			...
			fd_set exptfds;
			FD_ZERO(&exptfds);
			FD_SET(accepter->_listen_sock, &readfds);
			FD_SET(accepter->_listen_sock, &exptfds);

			/*select to judge if listen_sock is ok to accept. if not ok, turn to next loop*/
			int fds = select(0, &readfds, NULL, &exptfds, &accepter->_timeout);
			if (fds <= 0)
			{
				if (fds < 0)
				{
			...
			if (!FD_ISSET(accepter->_listen_sock, &readfds))
			{
				continue;
			}
			...
			{
				printf("WSAAccept error, errno=%d\n", WSAGetLastError()); //print some error here
			}
		}
