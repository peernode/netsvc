...
namespace netsvc{
	...
	public:
		...
		/*start accepter with listen @port and @worker number with edge trigger, @arg will be passed to handler, @accept_timeout(ms) will used when accept.*/
		...
		int start(unsigned short port, int worker_num, void *arg = NULL, int accept_timeout = -1);

		/*start accepter with local @ip & listen @port and @worker number with edge trigger, @arg will be passed to handler, @accept_timeout(ms) will used when accept.*/
		int start(unsigned int ip, unsigned short port, int worker_num, void *arg = NULL, int accept_timeout = -1);
	...
	template<class handler>
	int epoll_accepter<handler>::start(unsigned short port, int worker_num, void *arg /* = NULL */, int accept_timeout /* = -1 */)
	{
		return start(INADDR_ANY, port, worker_num, arg, accept_timeout);
	}

	template<class handler>
	int epoll_accepter<handler>::start(unsigned int ip, unsigned short port, int worker_num, void *arg/*=NULL*/, int accept_timeout /* = -1 */)
	{
		if(worker_num <=0 )
			return -1;
		...
		/*set accept_timeout for _listen_sock*/
		if ( accept_timeout >= 0 )
		{
	...
	void* epoll_accepter<handler>::accept_thread(void *arg)
	{
		...
		while (!accepter->_accepter_stop)
		{
			...
			{
				;//if start with accept_timeout, accept will return invalid_sock when timeout; and do noting
			}
		}
